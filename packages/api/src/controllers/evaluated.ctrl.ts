...
class EvaluatedController {
  ...
  async list (req: IRequest, resp: Response) {
    const { page, rowsPerPage, filter, search } = req.query as { [key: string]: unknown };
    try {
      const results = await evaluatedServices
        .getByEvaluationRefWithTotalCount(req.params.evaluationRef, filter, search as string, page as number, rowsPerPage as number);
      resp.send(results);
    } catch (error) {
