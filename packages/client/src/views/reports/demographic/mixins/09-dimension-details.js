            ...
            if (this.segmentedAnswers[segKey].count) {
              ...
              for (const qKey of Object.keys(segmentVariable.questions)) {
                if (overalCounter === 0) {
                  ...
                  headerCols.push({
                    text: this.evaluationData.questionnaire.evaluations[dimKey][segmentVariable.name][segmentVariable.questions[qKey].name].reference[this.user.lang],
                    margin: [0, 14, 0, 0],
                    fontSize: 6.5,
                    color: '#888888',
                    alignment: 'center',
